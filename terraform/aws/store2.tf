resource "aws_instance" "Store2_permanent_peer" {
  connection {
  ...
  tags {
    Name          = "Store2_permanent_peer_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
  ...
  provisioner "file" {
    content     = "${data.template_file.permanent_peer_hab_sup_Store2.rendered}"
  ...
  provisioner "file" {
    content   = "${data.template_file.store2_register_app_toml.rendered}"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo hostname store2-permanentpeer-${random_id.instance_id.hex}",
      "sudo groupadd hab",
...
# Register instances
resource "aws_instance" "Store2_Register" {
  connection {
    user        = "${var.aws_ami_user}"
    private_key = "${file("${var.aws_key_pair_file}")}"
  }
  ...
  tags {
    Name          = "Store2_Register${count.index}_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Application = "${var.tag_application}"
  ...
  provisioner "file" {
    content     = "${data.template_file.register_hab_sup_Store2.rendered}"
    destination = "/home/${var.aws_ami_user}/hab-sup.service"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo rm -rf /etc/machine-id",
      "sudo systemd-machine-id-setup",
      "sudo hostname store2-register${count.index}-${random_id.instance_id.hex}",
      "sudo groupadd hab",
      "sudo adduser hab -g hab",
      "chmod +x /tmp/install_hab.sh",
      "sudo /tmp/install_hab.sh",
      ...
      "sudo systemctl enable hab-sup",
      "sleep ${var.sleep}",
      "sudo mkdir -p /hab/user/haproxy/config /hab/user/${var.config_package}/config /hab/user/${var.audit_package}/config /hab/logs",
      "if test ${count.index} -eq 1; then sudo touch /hab/logs/register.log; fi",
      "sudo chown hab:hab -R /hab/user",
      "sudo /sbin/sysctl -w net.ipv4.conf.all.accept_source_route=0",
      "sudo /sbin/sysctl -w net.ipv4.conf.default.accept_source_route=0",
      "sudo /sbin/sysctl -w net.ipv4.conf.default.accept_redirects=0",
      ...
      "sudo hab svc load -u ${var.bldr_url} ${var.audit_origin}/${var.audit_package} --group ${var.group} --strategy at-once --channel ${var.audit_channel}",
      "sudo hab svc load -u ${var.bldr_url} ${var.config_origin}/${var.config_package} --group ${var.group} --strategy at-once --channel ${var.config_channel}", 
      "sudo hab svc load -u ${var.bldr_url} ${var.reg_app_origin}/${var.reg_app_package} --group ${var.group} --strategy at-once --channel ${var.store2_channel}"
...
resource "aws_instance" "Store2_Backoffice" {
  connection {
    user        = "${var.aws_ami_user}"
    private_key = "${file("${var.aws_key_pair_file}")}"
  }
  ...
  tags {
    Name          = "Store2_Backoffice_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Application = "${var.tag_application}"
  ...
  provisioner "file" {
    content     = "${data.template_file.backoffice_hab_sup_Store2.rendered}"
    destination = "/home/${var.aws_ami_user}/hab-sup.service"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo rm -rf /etc/machine-id",
      "sudo systemd-machine-id-setup",
      "sudo hostname store2-backoffice-${random_id.instance_id.hex}",
      "sudo groupadd hab",
      "sudo adduser hab -g hab",
      "chmod +x /tmp/install_hab.sh",
      "sudo /tmp/install_hab.sh",
      ...
      "sudo hab svc load -u ${var.bldr_url} ${var.audit_origin}/${var.audit_package} --group ${var.group} --strategy at-once --channel ${var.audit_channel}",
      "sudo hab svc load -u ${var.bldr_url} ${var.config_origin}/${var.config_package} --group ${var.group} --strategy at-once --channel ${var.config_channel}", 
      "sudo hab svc load -u ${var.bldr_url} ${var.backoffice_app_origin}/${var.backoffice_app_package} --group ${var.group} --strategy at-once --channel ${var.store2_channel}"
