# Register instances
resource "aws_instance" "QA_Gen1_Register" {
  connection {
  ...
  tags {
    Name          = "QA_Gen1_Register_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
  ...
  provisioner "file" {
    content     = "${data.template_file.register_hab_sup_QA_Gen1.rendered}"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo hostname qa-gen1-register-${random_id.instance_id.hex}",
      "sudo groupadd hab",
      ...
      "sudo hab svc load -u ${var.bldr_url} ${var.reg_app_origin}/${var.reg_app_package} --group ${var.group} --strategy at-once --channel ${var.qa_channel}"
...
resource "aws_instance" "QA_Gen2_Register" {
  connection {
    user        = "${var.aws_ami_user}"
    private_key = "${file("${var.aws_key_pair_file}")}"
  }
  ...
  tags {
    Name          = "QA_Gen2_Register_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Application = "${var.tag_application}"
  ...
  provisioner "file" {
    content     = "${data.template_file.register_hab_sup_QA_Gen2.rendered}"
    destination = "/home/${var.aws_ami_user}/hab-sup.service"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo rm -rf /etc/machine-id",
      "sudo systemd-machine-id-setup",
      "sudo hostname qa-gen2-register-${random_id.instance_id.hex}",
      "sudo groupadd hab",
      "sudo adduser hab -g hab",
      "chmod +x /tmp/install_hab.sh",
      "sudo /tmp/install_hab.sh",
